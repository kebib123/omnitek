...
@section('breadcrumb')
<a href="{{ url('admin/associated/'.Request::segment(3).'/'.Request::segment(4).'/create') }}" class="btn btn-primary btn-sm">Create</a>
@endsection
@section('content')
          ...
          <tbody>
            ...
            <tr class="id{{$row->id}}">
              ...
              <td class="post_title title_hi_sh"> 
                ...
                <div class="row_actions"><span class="id">ID: {{$row->id}} | </span><span class="edit"><a href="{{url( 'admin/associated/'.Request::segment(3) .'/'. $row->id. '/edit')}}" aria-label="">Edit</a> 
                 </span>   |             
...
(function ( $ ) { 
  $('.submitdelete1').on('click',function(e){
    ...
    $.ajax({
      type:'delete',
      url:"{{url('admin/associated/').'/'.Request::segment(3).'/'}}" + id,
      data:{_token:csrf},    
